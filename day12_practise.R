...
# 2) iris 데이터를 7:3의 비율로 나눔
set.seed(1234)
idx <- sample(1:nrow(iris_z), nrow(iris_z)*0.7)
iris_train <- iris_z[idx,]
iris_test <- iris_z[-idx,]

iris_train_labels <- iris[idx,5]
iris_test_labels <- iris[-idx,5]
...
# 4) 테스트 데이터로 테스트(타깃 컬럼 : species)
table(iris_test_pred, iris_test_labels)
CrossTable(iris_test_pred,iris_test_labels)
sum(iris_test_labels==iris_test_pred)
...
submit <- test[c("PassengerId","Survived")]
submit
>>>>>>> b81a8339d1f35c54dea6ec952afe4e1349312aea
